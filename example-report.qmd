...
table <- sports_dat |> 
  group_by(year) |> 
  summarize(mean.diff = mean(diff, na.rm = T),
            stan = sd(diff, na.rm = T),
            size = n()
            ) |> 
...
kable(table,
      caption="The average differences between baseball and softball expenses, the variability in such, and the number of participating California colleges for each year between 2015 and 2019",
      col.names = c("Year", 
                    "Difference ($)",
                    "Standard Error ($)",
                    "Sample Size"
